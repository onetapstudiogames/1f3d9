...
- Canonical /window/live is a fixed surveyed cartographic plate inside the existing /window observatory, not a game viewport. After the complete lightweight directory loads, direct children receive natural non-grid plots append-stably in creation-ID order, so a later place takes open ground without moving an existing plot. Direct residents and named things spread through available room in stable positions. The selected place, every child place, and the immutable world show their stored drawings as individual sprites directly on surveyed ground; an unset or deliberately blank place adds no hatch, tile, or backing box. Resident and thing drawings use the shared lazy thumbnail route as sprites on that same ground: transparent pixels reveal the plate and no sprite has a backing card. Names sit below with a faint text shadow; an undrawn thing remains clickable as a small neutral marker plus name. Far zoom hides names, readable zoom shows them, and Follow is always named. Pointer hover or keyboard focus raises the complete item and opens one shared popover with drawing state, maker, current owner, 8 x 8 size, and latest action; no state, provenance, or ownership chips sit on sprites. On touch, the first tap raises and the second opens. Wheel or visible +/- zoom, two-pointer pinch, one-pointer or arrow-key pan, and visible Center controls stay between a hard 0.8 and 2.2. Center or 0 returns to scale 1 around a focused resident or raised item when one exists, otherwise around readable home ground for the current place; it never shrinks the whole survey into view. There is no Fit control or slider. Detailed plots paint only in and just beyond the camera while every farther plot stays a finger-sized reachable marker; Live never draws every detailed plot at once. Camera budgeting is presentation-only and changes no fixed ground, selection, exact count, or public record. Live redraws only when visible state, camera, or timed evidence changes; routes have no mounted animation outside the camera, and hiding the document or leaving Live cancels all Live animation. Phone Live has a CSS full-screen mode with a clear exit and Back support
...
- Opening /api/events?within_seconds=1800 automatically follows at most eight 200-row pages, or 1,600 events. If another page remains, Live keeps its verified cursor and offers Continue recent history; it does not call history complete until the viewer continues. Opening rows carry commit-safe change_id values, deduplicate with later /api/changes rows, and set final positions only: no replay, dashed trace, arrowhead, footstep, or bubble. Later visible-tab rows replay once in ascending change order per resident; the first successful catch-up after a hidden tab settles without stale replay. Every newly observed visible move/go_home animates for a distance-scaled 3.2 to 8 seconds; a busy batch may compress timing within that bound but never skips a resident. Follow, browser-local Focus, hovered residents, and the six movers nearest the camera receive doorway detail, eligible brief speech bubbles, and two or three footstep dots that fade after about two seconds; every other mover still moves as a simple constant-rate endpoint glide with no trail or bubble, and detail returns as the crowd thins. Only Follow receives the full accessible doorway polyline. Endpoints are recorded fact; doorway points and interpolation are browser presentation, not a city-recorded route. A note observed while watching may show an honest first-line bubble beside a detailed resident for a few seconds. Make gets one 600 ms place pulse; use places the resident beside and pulses only its displayed source_thing_id at the committed place_id. Missing or off-camera exact visuals are skipped, never guessed
...
- Every displayed place has an exact \`notes · N\` corner control from its bounded direct-room read. It opens an in-page newest-first panel for the whole direct-room history, 50 notes per page; only a human Continue follows the retained cursor, hidden tabs pause it, and sparse shared state is \`/window/live?place=<id>&notes=open\`. Clicking a live bubble opens that panel and scrolls to the exact full note. Live otherwise keeps bounded tree breadcrumbs, exact operable \`+N more\`, and explicit Show more. Thing names load through one bounded newest page and never auto-follow their cursor; resident census automatically follows at most eight pages, then retains a real Continue, with hidden tabs pausing it. Browser-local Focus stays in localStorage, never the URL or city; shareable Follow clears it and choosing Focus clears Follow. Reduced motion shows final positions and bounded live footstep, Follow-route, bubble, and use evidence without interpolation or pulse. One ALPHA chip remains until owner approval. Cut absolutely: sprite backing boxes, persistent opening traces, infinite/full-viewport terrain, a zoom slider, idle animation, looping sprites, claiming the finite doorway presentation is a recorded route, and any new dependency
- Vercel previews only expose a visible repeatable in-memory proof scene for a clean positions-only opening, transparent ground drawing, chip-free sprite and popover, paged notes panel with Continue, live bubble, zoom-dependent labels, 151 residents, 64 concurrent movers, exact thing use, crowding, Show more, forced failure, Retry, and reduced motion. Running it again resets the same scene; production exposes no proof control
