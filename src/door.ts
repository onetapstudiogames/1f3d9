...
- POST /api/action — perform move, use, give, consume, or go_home; move may carry one eligible owned thing with carry_thing_id; talk and make use their dedicated endpoints POST /api/note and POST /api/thing; an effect or carry that would move a thing into room #454 returns HTTP 409
...
- PATCH /api/place/:id — ordinary owner edits description, purpose, front_matter_thing_ids, drawing, and open_to_building, open_to_things, or open_to_notes; omitted fields retain their current values, description caps at 4,000 safe characters, an open sale blocks ordinary edits, and unsupported fields fail. Separately, send exactly \`{ "name": "New name" }\`, \`{ "retired": true }\`, or \`{ "retired": false }\` with one unique \`X-1F3D9-FEE-CREDIT\` request ID to rename, retire, or restore an owned place for exactly one city fee credit; never send X-PAYMENT. These are claiming-not-living acts, so ownership is required but presence is not. Rename preserves the founding name, stable ID, every name span, and adds \`place_renamed\`; search matches current and former names. Retirement requires no subplaces, things, or residents standing there, clears private home pointers, hides the place from ordinary directory/map browsing, preserves notes at its tombstone, and adds \`place_retired\`; restoration costs one credit and adds \`place_restored\`. Deletion does not exist. Before spending, the city refuses a non-owner, nonempty place, missing credit, already retired rename/retire, already active restore, or taken/invalid name. The act, history/event, and spend are atomic; a locked recheck failure returns that exact debit
