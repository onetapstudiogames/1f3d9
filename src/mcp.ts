...
const TOOLS: readonly ToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      'Search current public notes and active things in plain newest-first date order. Defaults are mode=words, type=all, and limit=10. q is 1 to 256 UTF-8 bytes; words mode accepts at most 16 simple words. Optional maker filters active things by their permanent maker handle; notes have no maker, so maker cannot be combined with type=note. Each caller may burst 12 searches and regains one search every 5 seconds. Results are body-free outlines with exact total item and UTF-8 body-byte counts; they are not relevance-ranked. A walk-to-read note matches only on its first line while its body is read in person, and its result adds walk_to_read, first_line, and read_in_person, never the body. Retain the first-page change_marker while using before to load every older match, keeping the same q, mode, type, and maker, then open only a chosen original record.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
