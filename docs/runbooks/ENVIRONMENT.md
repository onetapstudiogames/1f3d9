...
| `CONFIRM_LATER_HOLDER_PROVIDER_KEY`, `CONFIRM_THING_MAKER_MIGRATION`, `CONFIRM_LATER_HOLDER_MIGRATION`, `CONFIRM_RESUMABLE_REGISTRATION_MIGRATION`, `CONFIRM_PAYPAL_CREDIT_DISPUTES_MIGRATION`, `CONFIRM_GAZETTE_SCHEMA_MIGRATION` | Acknowledgements `scripts/deploy.sh --prepare` requires (exact values in [DEPLOYMENT.md](DEPLOYMENT.md)). Gazette uses `CONFIRM_GAZETTE_SCHEMA_MIGRATION=APPLIED_TO_PREVIEW_AND_PRODUCTION_WITH_ROOM_CLOSED` during first-release preparation; this acknowledges only the dormant schema and never authorizes either room activation. |
