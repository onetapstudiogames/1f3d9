...
- Canonical /window/live is a fixed surveyed cartographic plate inside the existing /window observatory, not a game viewport. After the complete lightweight directory loads, direct children receive fixed rectangular rooms in creation-ID order. Allocation is append-stable: a later place takes fresh ground at the parent edge and never moves an existing room. On arrival, a resident's ID deterministically chooses a free standing spot, so the opening arrangement agrees on reload; residents have no alphabetical assignment or fixed standing spot, never overlap another occupant, and keep at least half a sprite of clearance. Named things keep fixed specimen spots. A full room grows slightly into free ground at its parent's edge without moving any other room. Room membership and recorded moves are city facts; standing positions are browser presentation. The selected ordinary place tiles its own stored drawing at reduced opacity, or gets plain paper ground when unset; the immutable world tiles its stored founder-authored drawing the same way. Residents walk only between recorded endpoints above the fixed ground and plots. Pointer hover and keyboard focus bring a complete covered item and its complete label above every peer; on touch, the first tap raises that unit and the second opens it. Wheel or visible +/- zoom, two-pointer pinch, one-pointer or arrow-key pan, and visible Center controls stay between a hard 0.8 and 2.2. Center or 0 returns to scale 1 around a focused or followed resident or raised item when one exists, otherwise around readable home ground for the current place; it never shrinks the whole survey into view. There is no Fit control or slider. Detailed plots paint only in and just beyond the camera while every farther plot stays a finger-sized reachable marker; Live never draws every detailed plot at once. Camera budgeting is presentation-only and changes no fixed ground, selection, exact count, or public record. Phone Live has a CSS full-screen mode with a clear exit and Back support
