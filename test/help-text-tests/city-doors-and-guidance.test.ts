...
export function registerCityDoorsAndGuidanceTests(): void {
  ...
  test('contributor guidance names the current locked-decision count', () => {
    ...
    assert.deepEqual(recorded, Array.from({ length: 118 }, (_, index) => index + 1))
    assert.equal(recorded.at(-1), 118)
    assert.match(contributorGuide, /\(118 recorded decisions[^)]*do not relitigate locked\s+rows\)/u)
    ...
    assert.match(decisions, /\| 110 \|[^\n]*The After Room[^\n]*no deadline[^\n]*LOCKED/iu)
    assert.match(decisions, /\| 111 \|[^\n]*copy, reach, and convert[^\n]*supersedes only the brick-list sentence of #104[^\n]*LOCKED/iu)
    assert.match(decisions, /\| 112 \|[^\n]*copy itself[^\n]*not one of its owner's 20 free things[^\n]*growth_cap_per_day[^\n]*LOCKED/iu)
    ...
    assert.match(decisions, /\| 117 \|[^\n]*keeps its public action row[^\n]*only a destroy[^\n]*effect_scheduled[^\n]*LOCKED/iu)
    assert.match(decisions, /\| 118 \|[^\n]*at most 1,000 matching records[^\n]*totals_capped[^\n]*LOCKED/iu)
    ...
    assert.match(
      decisions,
      /\| 102 \|[^\n]*walk-to-read[^\n]*`read_here`[^\n]*retired place opens to everyone[^\n]*moderation reach is unchanged[^\n]*snapshots keep the full body[^\n]*42 tools[^\n]*41[^\n]*LOCKED/iu,
    )
    assert.match(
      decisions,
      /\| 103 \|[^\n]*walk-to-read note on its public first line[^\n]*never the body[^\n]*`walk_to_read` true or false[^\n]*supersedes only #102[^\n]*LOCKED/iu,
    )
    assert.match(
      decisions,
