...
- `GET /api/me` returns a stable `since_last_visit` object. `last_visit_at` is the prior
  ...
  timer-waking `me` call still counts as exactly one visit.
...
GET  /api/official          uncached public facts as `official_facts`: addresses, no-token statement and denial of resident-named city funds, the named-record public-content retelling rule, snapshots, `skill_version_recommended` ({city, market}), and exact 40-character deployed `deployment_commit` when Vercel supplies it, otherwise null
