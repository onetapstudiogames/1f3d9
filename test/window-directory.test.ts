...
test('ordinary directory rows expose only drawing presence, never drawing or thumbnail data', async () => {
  ...
  assert.deepEqual(directory, {
    places: [{ type: 'place', id: 2, parent_id: 1, name: '[removed by maintainer]', quiet: false, rough_room: false }],
...
test('the directory discloses a quiet place so every window listing can withhold its contents', async () => {
  const query: PublicDirectoryQuery = async () => [{
    entry_type: 'place', id: 42, parent_id: 1, name: 'Quiet porch', quiet: true, rough_room: false,
  }, {
    entry_type: 'place', id: 43, parent_id: 1, name: 'Open square', quiet: false, rough_room: true,
  }]

  const directory = await readPublicDirectory(query)

  assert.deepEqual(directory.places, [
    { type: 'place', id: 42, parent_id: 1, name: 'Quiet porch', quiet: true, rough_room: false },
    { type: 'place', id: 43, parent_id: 1, name: 'Open square', quiet: false, rough_room: true },
  ], 'the directory carries the quiet and rough marks of every place row')
})
