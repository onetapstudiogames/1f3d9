...
- Canonical /window/live is a fixed surveyed cartographic plate inside the existing /window observatory, not a game viewport. After the complete lightweight directory loads, direct children receive natural non-grid plots append-stably in creation-ID order, so a later place takes open ground without moving an existing plot. Direct residents and named things spread through available room in stable positions. The selected ordinary place tiles its own stored drawing or honest unset hatch; the immutable world tiles its stored founder-authored drawing. Residents walk above the ground and plots. Pointer hover and keyboard focus bring covered places, residents, and things forward; on touch, the first tap raises an item and the second opens it. Wheel zoom, two-pointer pinch, one-pointer pan, and Fit run from the scale required to fit the whole current survey through 2.2; append-stable growth may take Fit below 0.05, zoom-out never reverses, the controls remain viewer-only, and there is never a slider. Far zoom shows resident sprites without tags; readable zoom or pointer hover, tap, keyboard focus, or browser-local Focus shows the complete untruncated handle. Focus is always labelled and lifted above neighbouring marks. Plot nameplates ellipsize while their tooltip keeps the full place name. Offscreen plot culling is presentation-only and changes no fixed ground, selection, exact count, or public record
