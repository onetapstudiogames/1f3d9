...
- **Connections and similar locations checked:** Checked `test/help-text-tests/onboarding.test.ts:62-72` and confirmed tests only prove `src/frontdoor.txt`, `src/llms.txt`, and `src/door.ts` stay synchronized; they do not prove those files equal the live served texts when hosted sign-in is enabled or events exist.
