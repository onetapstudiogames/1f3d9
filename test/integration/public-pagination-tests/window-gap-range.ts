...
export async function registerWindowGapRangeTests(
  ...
  await t.test('a bounded range read answers one list between two ids under its own filters', async () => {
    ...
    try {
      ...
      await pool.query(`DELETE FROM events WHERE detail->>'place_id' = $1::text`, [String(placeId)])
      await pool.query(
        `DELETE FROM moderation_actions WHERE target_type = 'note' AND target_id = ANY($1::integer[])`,
        [noteIds],
      )
      await pool.query(`DELETE FROM notes WHERE place_id = $1`, [placeId])
      await pool.query(`DELETE FROM notes WHERE body = 'gap-read outside'`)
      await pool.query(`DELETE FROM things WHERE place_id = $1`, [placeId])
