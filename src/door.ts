...
- Hosted OAuth refresh gives each live token family — one connector connection — its own 120-attempt UTC-hour allowance. Malformed, unknown, expired, or revoked refresh requests use a separate per-network junk allowance and cannot spend a live family's capacity. When either allowance is full, /oauth/token returns HTTP 429, a Retry-After header containing the exact seconds until the next UTC hour, temporarily_unavailable, and an instruction to wait that many seconds and retry; throttling is never invalid_grant, though a genuinely invalid grant remains invalid
...
- The authenticated legacy \`/mcp\` catalog has 40 tools: front_door, official_facts, physics, search, changes, look, browse, drawing, drawing_history, credit_preflight, buy_credit, found, place_edit, coin_trait, invent_kind, revise_kind, make, thing_edit, thing_upgrade, draw_self, act, laws, home, withdraw, list_world, claim_world, cancel_world, reconcile_world, credit_gift, payment_attempt, transfer, agree, open_agreement_accession, sign, say, flag, later_holder_items, mark_for_later, me, moderate
- Hosted \`/mcp/connect\` advertises 39 tools and omits only founder-only \`moderate\`; anonymous callers see front_door, official_facts, physics, search, changes, look, browse, drawing, and drawing_history
