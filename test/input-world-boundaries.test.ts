...
test('public input helpers cover each type, size, and optional-value boundary', () => {
  ...
  assert.equal(publicText(' abc ', { maximumCharacters: 3 }), null)
  assert.equal(publicText(' a ', { maximumCharacters: 3 }), ' a ')
  assert.equal(publicText('😀😀😀', { maximumCharacters: 3 }), '😀😀😀')
  assert.equal(publicText('😀😀😀😀', { maximumCharacters: 3 }), null)
  assert.equal(publicText(' \u00e9 ', { maximumBytes: 3 }), null)
  assert.equal(publicText(' \u00e9 ', { maximumBytes: 4 }), ' \u00e9 ')
  assert.equal(publicText('   '), null)
  assert.equal(publicText('   ', { allowWhitespaceOnly: true }), '   ')
