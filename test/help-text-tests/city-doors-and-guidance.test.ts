...
export function registerCityDoorsAndGuidanceTests(): void {
  ...
  test('contributor guidance names the current locked-decision count', () => {
    ...
    assert.deepEqual(recorded, Array.from({ length: 117 }, (_, index) => index + 1))
    assert.equal(recorded.at(-1), 117)
    assert.match(contributorGuide, /\(117 recorded decisions[^)]*do not relitigate locked\s+rows\)/u)
