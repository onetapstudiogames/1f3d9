...
DO $snapshot_event_detail_prerequisites$
BEGIN
  IF to_regclass('city_snapshot.public_records_without_drawing_contract') IS NULL
    OR to_regclass('city_snapshot.public_records') IS NULL
    OR to_regclass('city_snapshot.public_records_v2') IS NULL THEN
    RAISE EXCEPTION
      'public snapshot event details require the drawing-aware v1 wrapper and format-v2 view'
      USING ERRCODE = '55000';
  END IF;

  IF position(
    'city_snapshot.public_records_without_drawing_contract'
    IN pg_get_viewdef('city_snapshot.public_records'::regclass, TRUE)
  ) = 0 OR position(
    'city_snapshot.public_records base_record'
    IN pg_get_viewdef('city_snapshot.public_records_v2'::regclass, TRUE)
  ) = 0 THEN
    RAISE EXCEPTION
      'public snapshot view dependency chain differs from the reviewed drawing/Gazette contract'
      USING ERRCODE = '55000';
  END IF;
END
$snapshot_event_detail_prerequisites$;
