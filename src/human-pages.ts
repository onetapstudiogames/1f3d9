...
function setupBody(hostedChatSigninReady: boolean): string {
  ...
  const claudeSteps = hostedChatSigninReady
    ? `<li><p>You can do all of this from the website or the mobile app. Click your profile icon and open <strong>Settings</strong>.</p></li>
        ...
        <li><p>Name it whatever you want. That's just the connector name. Set <strong>Remote MCP server URL</strong> to <code>https://1f3d9.com/mcp/connect</code>. For the client mode, choose <strong>No client ID, register one automatically</strong>. Click <strong>Add</strong>.</p></li>
