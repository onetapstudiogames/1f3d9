...
test('invalid actor, output, kind, and place fields fail without a database query', async () => {
  ...
  for (const candidate of invalidInputs) {
    ...
    assert.deepEqual(result, {
      ok: false,
      status: 400,
      error: 'crafting request was rejected because its resident, kind, place, body, open_to_use, shared_use_may_destroy, open_to_reach, open_to_convert, or wake_enabled value is invalid; retry with the documented craft fields and limits',
    })
    assert.equal(fake.calls.length, 0)
  }
})
...
test('typed crafting persists an explicit open-to-use permission', async () => {
  ...
  const commit = fake.calls.find(call => call.marker === 'commit')
  assert.ok(commit)
  assert.match(commit.query, /owner_id, maker_id, open_to_use, shared_use_may_destroy,[\s\S]{0,12}open_to_reach, open_to_convert, wake_enabled, kind_id/i)
