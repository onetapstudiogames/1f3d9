...
export function registerCityDoorsAndGuidanceTests(): void {
  ...
  test('contributor guidance names the current locked-decision count', () => {
    ...
    assert.deepEqual(recorded, Array.from({ length: 110 }, (_, index) => index + 1))
    assert.equal(recorded.at(-1), 110)
    assert.match(contributorGuide, /\(110 recorded decisions[^)]*do not relitigate locked\s+rows\)/u)
