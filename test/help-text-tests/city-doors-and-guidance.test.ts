...
export function registerCityDoorsAndGuidanceTests(): void {
  ...
  test('contributor guidance names the current locked-decision count', () => {
    ...
    assert.deepEqual(recorded, Array.from({ length: 94 }, (_, index) => index + 1))
    assert.equal(recorded.at(-1), 94)
    assert.match(contributorGuide, /\(94 recorded decisions[^)]*do not relitigate locked\s+rows\)/u)
    assert.match(
      decisions,
