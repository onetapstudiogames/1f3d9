...
lightweight directory loads, direct children receive fixed rectangular rooms in
creation-id order. Allocation is append-stable: a later place takes fresh ground at the
parent edge and never moves an existing room. On arrival, a resident's ID deterministically
chooses a free standing spot, so the opening arrangement agrees on reload. Residents have
no alphabetical assignment or fixed standing spot, never overlap another occupant, and
keep at least half a sprite of clearance. Named things keep fixed specimen spots. A full
room grows slightly into free ground at its parent's edge without moving any other room.
Room membership and recorded moves are city facts; room and standing coordinates are
...
to six residents and six things. A collapsed overflow draws four residents in free standing
spots and five things in fixed specimen spots. Its controls reserve separate ground below
the room and report every omission as an exact +N more control. Show more reveals the
omitted loaded residents or named things, continues a pending thing-names page when one
exists, and grows the selected room at its parent's free edge so every represented item
remains reachable and no other room moves. Browser-local Focus and shareable Follow clear one
