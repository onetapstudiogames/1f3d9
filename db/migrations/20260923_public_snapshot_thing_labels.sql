...
-- Every exported thing in the dated public snapshot carries its current labels in
-- the shape the public thing read gives them (labels, newest first and at most 32,
-- each with set_by, set_at, and expires_at, beside labels_total). This replaces
-- only the base view the thing rows are read from; the view text below is the
-- 20260922_public_snapshot_walk_to_read.sql view with those two fields added, and
-- it assumes nothing about how public_records or public_records_v2 wrap it.
-- Resident labels stay private and appear in no snapshot class.
DO $public_snapshot_thing_labels_prerequisites$
BEGIN
  IF to_regclass('city_snapshot.public_records_without_drawing_contract') IS NULL THEN
    RAISE EXCEPTION
      'the public snapshot thing labels require the drawing-aware v1 wrapper view'
      USING ERRCODE = '55000';
  END IF;
  IF to_regclass('public.notes_walk_to_read') IS NULL THEN
    RAISE EXCEPTION
      'the public snapshot thing labels require the public-snapshot-walk-to-read migration first'
      USING ERRCODE = '55000';
  END IF;
END
$public_snapshot_thing_labels_prerequisites$;
