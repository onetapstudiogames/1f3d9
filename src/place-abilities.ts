...
export function parsePlaceDials(body: Readonly<Record<string, unknown>>): DialParse {
  ...
  for (const [field, key] of [
    ...
    if (typeof body[field] !== 'boolean') {
      return refuse('allow_arriving_copies, wake_visitors, and rough_room must be boolean when present')
    }
    dials[key] = body[field]
  }
  if (body.growth_cap_per_day !== undefined) {
    const cap = wholeNumber(body.growth_cap_per_day, 0, GROWTH_CAP_MAX)
    if (cap === null) return refuse(`growth_cap_per_day must be a whole number from 0 to ${GROWTH_CAP_MAX}`)
  ...
  if (body.growth_share_per_family !== undefined) {
    ...
    if (share === null) return refuse(`growth_share_per_family must be a whole number from 1 to ${GROWTH_CAP_MAX}`)
