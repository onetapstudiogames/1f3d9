...
export async function markPaymentAttemptNeedsReview(
  database: PaymentAttemptQueryable,
  input: { publicId: string; leaseOwner: string; reason: string },
): Promise<PaymentAttemptRecord> {
  const review = paymentAttemptFromRow((await database.query(`
  ...
  const current = paymentAttemptFromRow((await database.query(`
...
function paymentIdentifier(payload: Record<string, unknown>): string | null {
  const extensions = objectValue(payload.extensions)
  const extension = extensions ? extensions['payment-identifier'] : null
  if (typeof extension === 'string' && extension.trim()) return extension.trim()
  const nested = objectValue(extension)
  const identifier = nested ? nested.id : null
  return typeof identifier === 'string' && identifier.trim() ? identifier.trim() : null
}

function legacyOperation(purpose: string): PaymentAttemptRecord['operation'] {
  if (purpose === 'sale') return 'direct_sale'
  if (purpose === 'frontier' || purpose === 'kind_invention' || purpose === 'kind_revision') return purpose
  return 'legacy'
}
...
export async function startX402PaymentAttempt(
  ...
  try {
    const decoded = JSON.parse(Buffer.from(draft.paymentHeader, 'base64').toString('utf8'))
    if (!decoded || typeof decoded !== 'object' || Array.isArray(decoded)) {
      return { error: 'X-PAYMENT header is not base64 JSON' }
    }
    paymentPayload = decoded as Record<string, unknown>
  } catch {
    return { error: 'X-PAYMENT header is not base64 JSON' }
  }
  const payer = walletValue(objectValue(paymentPayload.payload)?.authorization != null
    ? (objectValue(objectValue(paymentPayload.payload)?.authorization)?.from ?? null)
    : null)
  if (!payer) return { error: 'X-PAYMENT must contain a valid payer address' }
  const id = paymentIdentifier(paymentPayload)
  if (!id) return { error: 'X-PAYMENT must include a payment-identifier for safe retries' }
  try {
    const result = await createOrReadPaymentAttempt(database, {
      actorId: draft.actorId,
      operation: legacyOperation(draft.purpose),
      targetKey: `legacy:${draft.purpose}:${draft.actorId}:${id}`,
      request: { payment_identifier: id, purpose: draft.purpose },
      method: 'x402',
      network: 'base',
      token: draft.accepted.asset.toLowerCase(),
      payerWallet: payer,
      payeeWallet: draft.payeeWallet.toLowerCase(),
      amountUnits: BigInt(Math.round(draft.amountUsdc * 1e6)),
      x402Nonce: `0x${sha256Hex(id)}`,
      x402PayloadDigest: sha256Hex(JSON.stringify(paymentPayload)),
    }, () => sha256Hex(`legacy:${payer}:${id}`))
    return {
      attempt: {
        paymentKey: result.attempt.publicId,
        transactionHash: result.attempt.txHash,
        payerWallet: result.attempt.payerWallet ?? payer,
      },
      paymentPayload,
      payerWallet: payer,
    ...
    if (error instanceof PaymentAttemptConflictError) {
      return {
        error: error.message === 'payment attempt is already bound to different immutable terms'
          ? 'payment-identifier is already bound to different immutable terms'
          : error.message,
...
): Promise<{ paymentKey: string; transactionHash: string | null; payerWallet: string } | null> {
  const row = paymentAttemptFromRow((await runQuery(database, `
    /* payment-attempts:legacy-settled */
    UPDATE payment_attempts
    SET tx_hash = coalesce(tx_hash, lower($2)),
      status = CASE WHEN status = 'completed' THEN status ELSE 'payment_pending' END,
      updated_at = clock_timestamp()
    WHERE public_id = $1
      AND (tx_hash IS NULL OR tx_hash = lower($2))
    RETURNING *
  `, [paymentKey, transactionHash]))[0] as Record<string, unknown> | undefined)
  return row ? {
    paymentKey: row.publicId,
...
export async function markX402PaymentCompleted(
  ...
  completion: {
    ...
    completionKind: 'place' | 'kind_revision' | 'transfer_offer' | 'world_offer'
    completionId: number
    completionRevision?: number | null
  },
): Promise<{ paymentKey: string; transactionHash: string | null; payerWallet: string } | null> {
  const row = paymentAttemptFromRow((await runQuery(database, `
    /* payment-attempts:legacy-completed */
    UPDATE payment_attempts
    SET tx_hash = coalesce(tx_hash, lower($2)),
      status = 'completed',
      result_json = jsonb_build_object(
        'completion_kind', $3::text,
        'completion_id', $4::integer,
        'completion_revision', $5::integer
      ),
      completed_at = coalesce(completed_at, clock_timestamp()),
      updated_at = clock_timestamp()
    WHERE public_id = $1
