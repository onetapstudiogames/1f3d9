...
-- Decision #103: every exported note in the dated public snapshot carries
-- walk_to_read, true or false, beside its unchanged body. This replaces only the
-- base view the note rows are read from; the view text below is the
-- 20260902_public_snapshot_quiet.sql view with that one field added, and it
-- assumes nothing about how public_records or public_records_v2 wrap it.
DO $public_snapshot_walk_to_read_prerequisites$
BEGIN
  IF to_regclass('city_snapshot.public_records_without_drawing_contract') IS NULL THEN
    RAISE EXCEPTION
      'the public snapshot walk-to-read mark requires the drawing-aware v1 wrapper view'
      USING ERRCODE = '55000';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION
      'the public snapshot walk-to-read mark requires the note-walk-to-read migration first'
      USING ERRCODE = '55000';
  END IF;
END
