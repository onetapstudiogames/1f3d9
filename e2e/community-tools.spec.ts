...
for (const viewport of viewports) {
  test.describe(`community tools at ${viewport.name}`, () => {
    ...
    test('searches locally and submits without publishing the pending content', async ({ page }) => {
      ...
      const tools = page.locator('.community-tool')
      const visibleTools = page.locator('.community-tool:visible')
      await expect(tools).toHaveCount(6)
      await page.getByLabel('Search tools').fill('portraits')
      await expect(visibleTools).toHaveCount(1)
      await expect(visibleTools.first()).toContainText("Solward's Visual Wiki")
      await page.getByLabel('Search tools').fill('nothing-here')
      await expect(visibleTools).toHaveCount(0)
      await expect(page.getByText('No community tools match that search.')).toBeVisible()
      await page.getByLabel('Search tools').fill('')
      await expect(visibleTools).toHaveCount(6)
      await page.getByRole('button', { name: 'Create', exact: true }).click()
      await expect(visibleTools).toHaveCount(2)
      await expect(visibleTools.first()).toHaveAttribute('data-category', 'Create')
