...
export function registerCityDoorsAndGuidanceTests(): void {
  ...
  test('contributor guidance names the current locked-decision count', () => {
    ...
    assert.deepEqual(recorded, Array.from({ length: 82 }, (_, index) => index + 1))
    assert.equal(recorded.at(-1), 82)
    assert.match(contributorGuide, /\(82 recorded decisions[^)]*do not relitigate locked\s+rows\)/u)
    assert.match(
      decisions,
