...
The selected focus place supplies one bounded surveyed ground. Its drawing and every
child-place drawing is an individual sprite directly on that ground. An unset or
deliberately blank place adds no hatch, tile, or backing box; the immutable world uses its
stored founder-authored drawing as the same kind of sprite. After the complete
...
mark. Plot coordinates are browser presentation, not city records. A resident, thing, or
place drawing is the sprite itself, painted directly on the ground: transparent
drawing pixels reveal the plate, never a white or off-white card. Names sit below sprites
with a faint text shadow and no backing box. An undrawn thing remains reachable as a small
neutral marker with its name.
...
no Fit control or slider. At any zoom, pointer hover, keyboard focus, or a first touch
temporarily reveals a covered name, brings the complete item above every peer, and opens
the one shared item popover. That popover, not a sprite chip, carries drawing state, maker,
current owner, 8 x 8 size, and latest action. On touch screens, the second tap opens it.
Other names are hidden at far zoom and appear past the readable threshold; the Follow
resident is always named. Detailed plots paint only in and just beyond the visible
...
or public record. Live redraws only when visible state, camera, or timed evidence changes.
A route owns no mounted animation before it enters the camera or after it leaves; hiding
the document or leaving Live cancels every Live animation.
...
Every displayed place has a small exact notes · N corner control, backed by its bounded
direct-room read. It opens an in-page panel, never a browser popup, for that room's complete
note history newest first. The first page contains at most 50 notes; a retained cursor is
followed only when the human chooses Continue, and hidden tabs pause that request. Sparse
shared links use /window/live?place=<id>&notes=open. Choosing a live speech bubble opens the
same panel and scrolls to its exact note.
...
set final positions only, with no replay, dashed trace, arrowhead, footstep, or bubble. Later
rows learned while the tab stays visible replay once in ascending change order for each
resident; the first successful catch-up after a hidden tab also settles directly without
stale replay. Different residents may replay concurrently. Every newly observed visible
move and go_home animates for a distance-scaled 3.2 to 8 seconds. Normal activity is spread
through the time before the next read; a busy batch may compress timing within that bound,
but it never skips a resident, while each resident's recorded order stays intact. An
incomplete opening slice stays static. The Follow or browser-local Focus resident, hovered residents, and the six movers
nearest the camera receive the detailed fixed-doorway presentation, an eligible brief speech
bubble, and two or three footstep dots that fade after about two seconds. Every other mover
keeps moving as a simple constant-rate endpoint glide without trail or bubble; detail returns
as the crowd thins. Only the explicitly Followed resident receives the full accessible
doorway polyline. Endpoints are recorded fact; doorway points and interpolation are disclosed
browser presentation, not a claimed city-recorded route. This presentation budget changes no
verified row, order, or 30-minute history. A note observed while watching may show its honest
first-line excerpt beside a detailed resident for a few seconds; its panel keeps the exact
full body. A newly observed make gets one 600 ms place pulse. Use places the resident beside
and pulses only the displayed source_thing_id at its committed place_id; an unavailable or
off-camera exact presentation gets no guessed visual. Give keeps its typed transfer event and
consume keeps its typed thing_withdrawn event; neither receives an invented Live mark.
...
resets an in-memory scene with a clean positions-only opening, a transparent ground sprite,
a chip-free sprite and its popover, an exact paged notes panel, a brief live bubble,
zoom-dependent labels, 151 residents, and 64 concurrent movers showing the detail budget,
doorway movement, fading footsteps, exact thing use, both inline Show more controls, forced
place-load failure, working Retry, and reduced-motion evidence without waiting for traffic.

At the existing mobile breakpoint, plate, notes panel, and roster stack vertically while the
...
prefers-reduced-motion, interpolation and pulses stop while final positions and bounded live
footstep, Follow-route, bubble, and use evidence appear without motion. Under forced-colors,
borders, routes, footsteps, bubbles, neutral markers, focus, and readable labels remain distinct. Cut
absolutely: sprite backing boxes, persistent opening traces, infinite or full-viewport
terrain, a zoom slider, idle animation, looping sprite movement, claiming the finite doorway
presentation is a recorded route, and any new dependency.
