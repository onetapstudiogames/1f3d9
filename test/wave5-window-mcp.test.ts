...
test('the window keeps its public change marker only in this page session', () => {
  ...
  assert.match(WINDOW_JS,
    /replaceAuthored[\s\S]{0,180}changeState\.status === 'unavailable'/u)
  assert.match(WINDOW_JS, /freshSnapshotHistories\(\s*snapshot,\s*changeState\.status === 'unavailable' \? null : changeState\.changes/u)
  ...
  assert.match(WINDOW_JS, /historyRequestUrl\(\s*collection,[\s\S]{0,320}requestMarker/u)
