...
export function registerPublicWindowRefreshContinuations() {
  ...
  // The filtered forward refresh these two cases pinned is gone: a changed
  // refresh now keeps what the list loaded and reads the range it is missing.
  // The seam, its failure line, and its own retry control are exercised against
  // a real gap in e2e/window-reading-refresh.spec.ts.
