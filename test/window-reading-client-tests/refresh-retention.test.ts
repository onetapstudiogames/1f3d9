...
export function registerRefreshRetentionTests(): void {
  ...
  test('a filtered list starts from its own newest page, never the citywide newest page', () => {
    const loaded = rowsFrom(100, 25).map(row => ({ ...row, place_id: 77 }))
    const previous = {
      notes: { 'place:77|resident:': loadedEntry(loaded, { filters: { placeId: 77 } }) },
      things: {}, agreements: {}, events: {},
    }
    const citywide = rowsFrom(500, 50).map(row => ({ ...row, place_id: 1 }))
    const ownNewest = rowsFrom(150, 50).map(row => ({ ...row, place_id: 77 }))
    const ownNewestPages = {
      notes: { 'place:77|resident:': {
        failed: false,
        rows: ownNewest,
        page: { hasMore: true, nextBeforeId: 101 },
      } },
    }

    const entry = refreshedHistories(previous, snapshotOf({ notes: citywide }), {
      ownNewestPages,
    }).notes!['place:77|resident:']!

    assert.deepEqual(ids(entry.rows).slice(0, 3), [150, 149, 148])
    assert.ok(entry.rows.some(row => row.id === 76), 'the list keeps its loaded bottom')
    assert.ok(!entry.rows.some(row => row.id === 500), 'a citywide row never enters the list')
  })

  test('a context neighbor absent from the next own page is not called removed', () => {
    const previous = {
      notes: { 'context|place:77|resident:alice': loadedEntry([
        { id: 100, place_id: 77, author: 'alice' },
        { id: 99, place_id: 77, author: 'bob' },
      ], { filters: { placeId: 77, resident: 'alice', context: true } }) },
      things: {}, agreements: {}, events: {},
    }
    const ownNewestPages = {
      notes: { 'context|place:77|resident:alice': {
        failed: false,
        rows: [{ id: 110, place_id: 77, author: 'alice' }],
        page: { hasMore: true, nextBeforeId: 110 },
      } },
    }

    const entry = refreshedHistories(previous, snapshotOf({ notes: [] }), { ownNewestPages })
      .notes!['context|place:77|resident:alice']!

    assert.ok(entry.rows.some(row => row.id === 99), 'the neighbor ride-along remains loaded')
    assert.equal(entry.nextBeforeId, 100, 'only the followed resident sets the cursor')
  })

  test('every stale loaded row leaves an explicit range even with no surviving marker', () => {
    const previous = {
      notes: { 'place:77|resident:': loadedEntry([
        { id: 100, place_id: 77 }, { id: 99, place_id: 77 },
      ], { filters: { placeId: 77 } }) },
      things: {}, agreements: {}, events: {},
    }
    const ownNewest = rowsFrom(200, 50).map(row => ({ ...row, place_id: 77 }))
    const ownNewestPages = {
      notes: { 'place:77|resident:': {
        failed: false,
        rows: ownNewest,
        page: { hasMore: true, nextBeforeId: 151 },
      } },
    }
    const changes = [99, 100].map(target_id => ({
      kind: 'moderation', detail: { target_type: 'note', target_id },
    }))

    const entry = refreshedHistories(previous, snapshotOf({ notes: [] }), {
      changes, ownNewestPages,
    }).notes!['place:77|resident:']!

    assert.ok(!entry.rows.some(row => row.id === 99 || row.id === 100),
      'known stale text is withheld')
    assert.deepEqual(entry.gapRanges, [{ beforeId: 151, afterId: 98 }],
      'the retry includes both stale ids even without a surviving lower row')
  })

  test('a failed own-newest read keeps a seam for stale text withheld inside the list', () => {
    const previous = {
      notes: { 'place:77|resident:': loadedEntry([
        { id: 100, place_id: 77 }, { id: 99, place_id: 77 }, { id: 98, place_id: 77 },
      ], { filters: { placeId: 77 } }) },
      things: {}, agreements: {}, events: {},
    }
    const ownNewestPages = {
      notes: { 'place:77|resident:': { failed: true } },
    }
    const changes = [{
      kind: 'moderation', detail: { target_type: 'note', target_id: 99 },
    }]
    ...
    assert.deepEqual(ids(entry.rows), [100, 98], 'known stale text stays hidden')
    assert.equal(entry.newestReadFailed, true, 'the list offers an honest newest-page retry')
    assert.deepEqual(entry.gapAfterIds, [98],
      'the withheld internal row still leaves an exact seam after retry')
  })
  ...
  test('a refresh whose changes read failed keeps no older rows', () => {
    ...
    const filtered = histories.notes!['place:11|resident:']!
    assert.deepEqual(ids(filtered.rows), [],
      'a filtered list retains no older rows when changes cannot be checked')
    assert.equal(filtered.newestReadFailed, false,
      'its own completed newest page remains a safe fresh result')
  })
