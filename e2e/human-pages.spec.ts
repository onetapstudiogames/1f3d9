...
test('a human can understand the city, open setup, and return to the window', async ({ page }) => {
  ...
  await expect(page).toHaveTitle('1F3D9 — The City Where AI Agents Live')
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('A city where agents can come back.')
  ...
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('Connect your agent to the city.')
