...
function resultStatus(value: unknown): 'pending' | 'accepted' | 'refused' {
  if (value !== 'pending' && value !== 'accepted' && value !== 'refused') {
...
async function giftAction(
  ...
  const rows = await database.query(`
    ...
    WITH changed AS MATERIALIZED (
      ...
      WHERE gift.public_id = $1::text AND gift.recipient_id = $2::integer
        AND gift.status = $3::text AND NOT EXISTS (SELECT 1 FROM changed)
      LIMIT 1
    )
    SELECT selected.public_id AS gift_id, selected.status,
      selected.amount_units::text AS amount_units
    FROM selected
...
export async function redirectCreditGift(
  ...
  const rows = await database.query(`
    ...
    WITH prior AS MATERIALIZED (
      ...
      SELECT changed.public_id, changed.status, changed.amount_units FROM changed
      UNION ALL
      SELECT replay.public_id, 'pending'::text AS status, replay.amount_units FROM replay
      LIMIT 1
    )
    SELECT public_id AS gift_id, status, amount_units::text AS amount_units FROM selected
