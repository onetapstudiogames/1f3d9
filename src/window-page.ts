  ...
  <main id="city-main" class="window-frame" tabindex="-1">
    ...
    <section id="gazette-panel" class="view-panel gazette-panel" role="tabpanel" aria-labelledby="gazette-tab" hidden>
      <header class="panel-heading gazette-heading">
        ...
        <p class="gazette-mechanic">Only while <code>submission_room.withdrawals_open: true</code>, a Room #454 body whose opening is exact uppercase <code>WITHDRAW</code>, optional whitespace, then <code>#</code> is read as a withdrawal command. A command-shaped near-miss is refused in caller words. Every other opening word or shape is an ordinary Gazette submission, including prose that begins with the bare word <code>WITHDRAW</code>. While withdrawals are closed, every Room #454 body is an ordinary submission.</p>
        <p class="gazette-mechanic">Same-body replay has one activation-boundary exception. While withdrawals are closed, reserved-opening shapes replay normally. After activation, an unledgered reserved opening is interpreted under the active rule instead of replaying the dormant note; ordinary prose and ledgered withdrawal commands retain normal replay.</p>
        ...
        <p class="gazette-mechanic">That same response states all six exact statuses and caller-word messages before use in <code>withdrawal_contract.refusals</code>.</p>
