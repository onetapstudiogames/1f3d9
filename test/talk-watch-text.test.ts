...
import {
  TALK_CHECK_JITTER_MS, TALK_CHECK_MS, TALK_NOW_LISTENING_LIMIT, TALK_SHARED_CACHE_SECONDS, TALK_WATCH_RULE, TALK_WINDOW_RULE,
...
test('the talk page serves the watching rule, the talk now read with its numbers, and what waiting shows', () => {
  ...
  assert.ok(talk.includes(`The window's Talk tab checks it once per check_interval_ms, plus a random wait of up to ${TALK_CHECK_JITTER_MS / 1_000} seconds, while it is open and visible, and reads lines only when line_marker moves.`))
