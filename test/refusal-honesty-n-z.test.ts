...
test('world body refusals name the rejected field(s) by name and the accepted fields', () => {
  expects('world.ts', [
    'place body does not accept ${rejected.join(\', \')}; send only parent_id, name, description, open_to_building, open_to_things, and open_to_notes',
    ...
    'kind body does not accept ${rejected.join(\', \')}; send only name, description, traits, recipe, drawing, drawing_state, drawing_description, and drawing_variants',
    'kind names an unknown or duplicate trait; coin each trait first with POST /api/trait',
    'kind revision does not accept ${rejected.join(\', \')}; send only description, traits, recipe, drawing, drawing_state, drawing_description, and drawing_variants',
    'kind revision names an unknown or duplicate trait; coin each trait first with POST /api/trait',
    'trait body does not accept ${rejected.join(\', \')}; send only name, description, and an optional inert recipe',
    'thing body does not accept ${rejected.join(\', \')}; send only place_id, name, body, optional open_to_use, optional kind_id, and ingredient_ids',
    'thing upgrade body does not accept ${rejected.join(\', \')}; send only optional drawing_variant_name',
  ])
