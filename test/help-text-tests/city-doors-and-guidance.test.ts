...
export function registerCityDoorsAndGuidanceTests(): void {
  ...
  test('contributor guidance names the current locked-decision count', () => {
    ...
    assert.deepEqual(recorded, Array.from({ length: 95 }, (_, index) => index + 1))
    assert.equal(recorded.at(-1), 95)
    assert.match(contributorGuide, /\(95 recorded decisions[^)]*do not relitigate locked\s+rows\)/u)
    assert.match(
      decisions,
