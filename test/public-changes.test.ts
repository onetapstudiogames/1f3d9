...
test('public change query rejects duplicate, malformed, future-sized, and unknown input', () => {
  const cases = [
    ...
    [{ kind: ['note'] }, /kind requires since; omit kind, limit, and since to obtain a marker/iu],
    [{ limit: ['1'] }, /limit requires since; omit kind, limit, and since to obtain a marker/iu],
